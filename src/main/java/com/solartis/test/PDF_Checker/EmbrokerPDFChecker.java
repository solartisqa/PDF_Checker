...
public class EmbrokerPDFChecker  extends StarrGLPDFChecker 
{
    ...
    public void pumpDatatoForms(LinkedHashMap<Integer,SheduleOfFormsList> formsList,LinkedHashMap<String, String> inputOutputrow, PropertiesHandle config, String SampleLocation, String TempLocation) throws DatabaseException, PDFException
    {
    	for (Entry<Integer,SheduleOfFormsList> entryy : formsList.entrySet())	
		{
    		...
    		if(formlist.getFormNature().equalsIgnoreCase("dynamic"))
    		{
				...
				{
					LinkedHashMap<String, String> ConfigTableRow = entry.getValue();
					if(ConfigTableRow.get("FlagForExecution").equals("Y"))
		    		{
						...
						if(ConfigTableRow.get("TranslationFlag").equals("Y"))
			    		{
							pdf.feedInData(PageNumber, llx, lly, urx, ury, (String) Translation(ConfigTableRow));
			    		}
						else
						{
    ...
    public static void main(String args[]) throws IOException, DocumentException
    {
    	...
    	EmbrokerPDFChecker pdfcheck = new EmbrokerPDFChecker();
    ...
    protected String Translation(LinkedHashMap<String, String> ConfigTableRow)
	{
		String outputdata = null;
		System.out.println(ConfigTableRow.get("TranslationFunction"));
		switch(ConfigTableRow.get("TranslationFunction"))
		{
		case "AddressLinesFormat1":
			String[] columns = ConfigTableRow.get("ColumnName").split(";");
			outputdata = columns[0]+","+columns[1]+"\n"+columns[2]+","+columns[3]+" "+columns[4];
			break;
		case "EmbrokerAddressDec":
			String[] columns1 = ConfigTableRow.get("ColumnName").split(";");
			outputdata = columns1[0]+"\n"+columns1[1]+"\n"+columns1[2]+"\n"+columns1[3]+"\n"+columns1[4]+","+columns1[5]+" "+columns1[6];
			break;
		}
		return outputdata;
	}
