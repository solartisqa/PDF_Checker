...
public class Pdfcomparecheck<T extends CompareResult> {
    ...
    private Pdfcomparecheck(T compareResult) {
        Objects.requireNonNull(compareResult, "compareResult is null");
        this.compareResult = compareResult;
    }

    @SuppressWarnings("unchecked")
	public Pdfcomparecheck(String expectedPdfFilename, String actualPdfFilename) throws IOException {
        this(expectedPdfFilename, actualPdfFilename, (T) new CompareResult());
    }

    public Pdfcomparecheck(String expectedPdfFilename, String actualPdfFilename, T compareResult) throws IOException {
        this(compareResult);
    ...
    @SuppressWarnings("unchecked")
	public Pdfcomparecheck(final Path expectedPath, final Path actualPath) throws IOException {
        this(expectedPath, actualPath, (T) new CompareResult());
    }

    public Pdfcomparecheck(final Path expectedPath, final Path actualPath, final T compareResult) throws IOException {
        this(compareResult);
    ...
    @SuppressWarnings("unchecked")
	public Pdfcomparecheck(final File expectedFile, final File actualFile) throws IOException {
        this(expectedFile, actualFile, (T) new CompareResult());
    }

    public Pdfcomparecheck(final File expectedFile, final File actualFile, final T compareResult) throws IOException {
        this(compareResult);
    ...
    @SuppressWarnings("unchecked")
	public Pdfcomparecheck(final InputStream expectedPdfIS, final InputStream actualPdfIS) {
        this(expectedPdfIS, actualPdfIS, (T) new CompareResult());
    }

    public Pdfcomparecheck(final InputStream expectedPdfIS, final InputStream actualPdfIS, final T compareResult) {
        this(compareResult);
        ...
        drawExecutor.execute(() -> {
            try {
                ...
                diffExecutor.execute(() -> {
                   ...
                   // compareResult.
                    float resultImageWidth = Math.max(expectedImage.width, actualImage.width);
                    float resultImageHeight = Math.max(expectedImage.height, actualImage.height);
                    PageDiffCalculator diffCalculator= new PageDiffCalculator((int) (resultImageWidth*resultImageHeight),100);
                    if (diffCalculator.differencesFound()) 
                    {
File outputfile = new File("E:\\RestFullAPIDeliverable\\Devolpement\\admin\\STARR-GL\\PDFs\\PolicyPDF\\Result\\age"+pageIndex+".jpg");
try {
	ImageIO.write( diffImage.getImage(), "jpg", outputfile);
} catch (IOException e) {
    ...
    {
    	Pdfcomparecheck compares= new Pdfcomparecheck("E:\\RestFullAPIDeliverable\\Devolpement\\admin\\STARR-GL\\PDFs\\PolicyPDF\\Actual\\SepLCV_AL_01.pdf", 
				"E:\\RestFullAPIDeliverable\\Devolpement\\admin\\STARR-GL\\PDFs\\PolicyPDF\\Expected\\SepLCV_AL_01.pdf");
    	
    	System.out.println(compares.compare());
    	//compares.getResult().writeTo("E:\\RestFullAPIDeliverable\\Devolpement\\admin\\STARR-GL\\PDFs\\PolicyPDF\\Result\\diffOutput.pdf");
    }
}
*/